"""
File:    Fugacity Calculator w/ Peng & Robinson and Poynting Factor
...
Description: The goal of this calculator is to essentially calculate the fugacity value using the EOS of Peng & Robinson
                and adjust the values according to Poynting Factor
"""
import numpy as np
import math
...
def peng_robinson_w_poynting_factor(pressure_p, temperature_t, constant_r):
    """
    The Peng Robinson Equation of State and Poynting Factor to correct the value
    ...
    :return: Returns the needed Z-factor and Volume, and the corrected fugacity for the liquid phase since P/R is
                not good for that! For this, which it will be inaccurate for this assumption, we will take the smallest
                volume coming out of the Peng & Robinson EOS despite saturated liquids being it's most INACCURATE form
    """
    ...
    fugacity = P * math.exp((compressibility_z_3 - 1) -
                            ...
                            sym.log((compressibility_z_3 + (1 + 2 ** (1 / 2)) * cap_b) / (compressibility_z_3 +
                                                                                          (1 - 2 ** (1 / 2)) * cap_b)))
    # Peng & Robinson was meant to simply calculate the sat-liquid fugacity,
    # which is equivalent to the saturated fugacity!
    fugacity_sat = fugacity

    # taking the absolute value of the volume just so that it could avoid the very negligible imaginary number at end
    volume2 = abs(volume2[0])

    # the saturated pressure to go along with our given temperature, assuming our liquid in general is saturated
    P_sat = 1.0142

    """
    THIS IS THE FUGACITY OF THE LIQUID THAT IS MUCH MORE ACCURATE THANKS TO THE POYNTING FACTOR
    """
    fugacity = fugacity_sat * math.exp((volume2 * (P - P_sat)) / (R * T))
    print(fugacity)
    return fugacity
...
if __name__ == '__main__':
    ...
    while i != 10:
        ...
        list_of_fugacities[i] = peng_robinson_w_poynting_factor(list_of_terms[0], list_of_terms[1], list_of_terms[2])
    ...
    plt.title("Poynting Factor Fugacity v. Pressure")
    plt.xlim(5, 110, 10)
    plt.ylim(5, 265, 10)
    plt.show()